...
namespace
{
    ...
    {
        if ( cell == nullptr || cell->class_type != MAT_C_CELL )
            return;
    ...
    void
    ExerciseStruct(matvar_t *st, FuzzedDataProvider &fdp)
    {
        if ( st == nullptr || st->class_type != MAT_C_STRUCT )
            return;
        ...
        int idx_int = fdp.ConsumeIntegralInRange<int>(0, nfields > 0 ? (int)nfields - 1 : 0);
        ...
        if ( st->rank > 0 && st->rank <= 4 ) {
            ...
            if ( !any_zero_dim ) {
                matvar_t *got =
                    Mat_VarGetStructs(st, starts, strides, edges, fdp.ConsumeBool() ? 1 : 0);
                if ( got != nullptr )
                    Mat_VarFree(got);
            }

            if ( nelems > 0 ) {
                int lstart = fdp.ConsumeIntegralInRange<int>(0, (int)(nelems - 1));
                ...
                matvar_t *gotl =
                    Mat_VarGetStructsLinear(st, lstart, lstride, ledge, fdp.ConsumeBool() ? 1 : 0);
                if ( gotl != nullptr )
                    Mat_VarFree(gotl);
            }
        }
